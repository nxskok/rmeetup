...
author: "Ken Butler |
    | Department of Computer and Mathematical Sciences, UTSC (Scarborough) 
...
```{r}
prevalence_longer %>% 
  pivot_wider(names_from=c(disease, location), values_from=frequency)
```
...
```{r}
ggplot(repmes3, aes(x=fct_inorder(time), y=y, colour=treatment, group=id)) +
